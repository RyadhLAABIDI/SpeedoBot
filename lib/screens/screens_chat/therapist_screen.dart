...
class TherapistScreen extends StatefulWidget {
  ...
  @override
  _TherapistScreen createState() => _TherapistScreen();
}

class _TherapistScreen extends State<TherapistScreen> {
...
Future<void> _sendMessage() async {
  ...
  if (message.isNotEmpty) {
    setState(() {
      ...
      _isLoading = true; // Affiche le message de chargement
    ...
    // Ajout du message temporaire de réponse en cours
    setState(() {
      _messages.add('speedobot_response'.tr); // Message de chargement traduit
    });

    final url = Uri.parse('https://chat.speedobot.com/therapeute');
    try {
      final response = await http.post(
        url,
        headers: {'Content-Type': 'application/json'},
        body: json.encode({'problem': message}),
      ).timeout(const Duration(seconds: 120)); // Timeout après 2 minutes
      ...
      if (response.statusCode == 200) {
        ...
        if (reply.isEmpty) {
          ...
          // Nettoyage de la réponse avant l'affichage
          reply = _cleanResponse(reply);
        }

        setState(() {
          _messages.add(reply); // Ajoute la réponse nettoyée de l'API aux messages
          _isLoading = false; // Cache le message de chargement
        });
      } else {
        setState(() {
...
Widget _buildMessage(String message, int index) {
  ...
  return TextField(
    ...
    decoration: InputDecoration(
      hintText: 'compose_therapist_question'.tr,
       hintStyle: TextStyle(
