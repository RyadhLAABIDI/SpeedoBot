...
class ComposeEmailScreen extends StatefulWidget {
  ...
  @override
  _ComposeEmailScreen createState() => _ComposeEmailScreen();
}

class _ComposeEmailScreen extends State<ComposeEmailScreen> {
  final TextEditingController _controller = TextEditingController();
  final ScrollController _scrollController = ScrollController(); // Contrôleur de scroll
  List<String> _messages = [];  // Liste pour stocker les messages
  ...
  String _cleanResponse(String response) {
    ...
    response = _decodeUnicode(response);  // Conversion des caractères Unicode
    return response
        .split('\n')
        .map((line) => line.trim())
  ...
  Future<void> _sendMessage() async {
    ...
    if (message.isNotEmpty) {
      ...
      // Ajoute un message temporaire pour l'indicateur de réponse
      setState(() {
        _messages.add('response_loading'.tr);
      });

      final url = Uri.parse('https://chat.speedobot.com/ecrire_email');
      try {
        final response = await http.post(
          url,
          headers: {'Content-Type': 'application/json'},
          body: json.encode({'extra': message}), // Envoie le message de l'utilisateur
        ...
        if (response.statusCode == 200) {
          ...
          setState(() {
            _messages.add(reply);  // Ajoute la réponse (ou le message par défaut)
            _isLoading = false;
          });
        } else {
          setState(() {
            _messages.add('Erreur HTTP ${response.statusCode}');
            _isLoading = false;
          });
        }
      } catch (e) {
        setState(() {
          ...
          _messages.add('error_connection'.tr + ': ${e.toString()}');
          _isLoading = false;
        });
      }
    }
  ...
  // Fonction pour créer un fond dégradé avec des couleurs basées sur le thème
 ...
 Widget _buildMessageInputField() {
  ...
  return TextField(
    ...
    decoration: InputDecoration(
      hintText: 'compose_write_email_question'.tr,
       hintStyle: TextStyle(
