...
import 'dart:convert';
import 'package:get/get.dart'; // Assurez-vous d'avoir GetX dans pubspec.yaml
...
class ComputerScienceScreen extends StatefulWidget {
  ...
  @override
  _ComputerScienceScreen createState() => _ComputerScienceScreen();
}

class _ComputerScienceScreen extends State<ComputerScienceScreen> {
  ...
  Future<void> _sendMessage() async {
    ...
    if (message.isNotEmpty) {
      ...
      await _getComputerScienceResponse(message);
    }
  }

Future<void> _getComputerScienceResponse(String question) async {
  final url = Uri.parse('https://chat.speedobot.com/computerscience');
  ...
  try {
    ...
    setState(() {
      _messages.removeLast(); // Supprime le dernier message (message de chargement)
    });

    if (response.statusCode == 200) {
      final data = json.decode(response.body);
      String reply = data['response'] ?? ''; // Si réponse vide, mettre une chaîne vide

      // Si la réponse est vide, ajouter un message d'erreur professionnel
      if (reply.isEmpty) {
        reply = "error_no_reply".tr;
      } else {
        // Appliquer la méthode de nettoyage si la réponse n'est pas vide
        reply = _cleanResponse(reply);
      }

      setState(() {
        _messages.add('Speedobot : $reply'); // Ajoute la réponse nettoyée de l'API
      });
    } else {
      setState(() {
        _messages.add('Error: ${response.statusCode}'.tr); // Gestion des erreurs
      });
    }
  } catch (e) {
    setState(() {
      _messages.add('Failed to fetch response.'.tr); // Gestion des exceptions
    });
  }
}
 ...
 Widget _buildMessage(String message, int index) {
  ...
  return TextField(
    ...
    decoration: InputDecoration(
      hintText: 'compose_computer_science_question'.tr,
       hintStyle: TextStyle(
