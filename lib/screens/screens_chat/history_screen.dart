...
Future<void> _sendMessage() async {
  ...
  if (message.isNotEmpty) {
    ...
    await _getHistoryResponse(message);
  }
}

// Fonction pour envoyer la requête à l'API et obtenir la réponse
Future<void> _getHistoryResponse(String question) async {
  final url = Uri.parse('https://chat.speedobot.com/history'); // URL de l'API History
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      String rawResponseMessage = data['response'] ?? '';  // Si la réponse est vide, on met une chaîne vide
      ...
      setState(() {
        _messages.add('Speedobot : $rawResponseMessage');  // Ajoute la réponse nettoyée ou le message d'erreur
      });
    } else {
      setState(() {
        _messages.add('Error: ${response.statusCode}'.tr);  // Message d'erreur traduit en cas de statut HTTP différent de 200
      });
    }
  } catch (e) {
    setState(() {
      _messages.add('Failed to fetch response.');  // En cas d'échec de la requête
    });
  }
}
...
Widget _buildMessage(String message, int index) {
  ...
  return TextField(
    ...
    decoration: InputDecoration(
      hintText: 'compose_history_question'.tr,
       hintStyle: TextStyle(
