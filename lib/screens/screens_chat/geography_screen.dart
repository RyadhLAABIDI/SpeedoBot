...
class GeographyScreen extends StatefulWidget {
  ...
  @override
  _GeographyScreen createState() => _GeographyScreen();
}
...
Future<void> _sendMessage() async {
  ...
  if (message.isNotEmpty) {
    ...
    await _getGeographyResponse(message);
  }
}

// Fonction pour envoyer la requête à l'API et obtenir la réponse
Future<void> _getGeographyResponse(String question) async {
  final url = Uri.parse('https://chat.speedobot.com/geography');
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (rawResponseMessage.isEmpty) {
        rawResponseMessage = "error_no_reply".tr;  // Message d'erreur traduit
      } else {
      ...
      setState(() {
        _messages.add(': $rawResponseMessage');  // Ajoute la réponse nettoyée ou le message d'erreur
      });
    } else {
      setState(() {
        _messages.add('Error: ${response.statusCode}'.tr);  // Message d'erreur traduit en cas de statut HTTP différent
      });
    }
  } catch (e) {
    setState(() {
      _messages.add('error_no_reply'.tr);  // En cas d'échec de la requête
    });
  }
}
 ...
 Widget _buildMessageInputField() {
  ...
  return TextField(
    ...
    decoration: InputDecoration(
      hintText: 'compose_geography_question'.tr,
       hintStyle: TextStyle(
