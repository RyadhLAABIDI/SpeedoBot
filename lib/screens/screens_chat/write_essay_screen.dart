import 'dart:convert';  // Pour encoder et décoder les données JSON
...
class WriteEssayScreen extends StatefulWidget {
  const WriteEssayScreen({super.key});

  @override
  _WriteEssayScreen createState() => _WriteEssayScreen();
}

class _WriteEssayScreen extends State<WriteEssayScreen> {
...
Future<void> _sendMessage() async {
  ...
  if (message.isNotEmpty) {
    setState(() {
      _messages.add(message);  // Ajoute le message à la liste
      _controller.clear();  // Efface le champ de texte après envoi
    });

    // Scroll vers le bas lorsque le message est envoyé
    ...
    setState(() {
      // Message temporaire de Speedobot en attendant la réponse
      _messages.add('speedobot_response'.tr); // Message de chargement traduit
    });

    // API de l'essai : rediger_essai
    final url = Uri.parse('https://chat.speedobot.com/rediger_essai');
    try {
      final response = await http.post(
        url,
        headers: {'Content-Type': 'application/json'},
        body: json.encode({'extra': message}),
      ).timeout(const Duration(seconds: 120)); // Timeout après 120s

      print('Status Code: ${response.statusCode}'); // Debug
      print('Response Body: ${response.body}');    // Debug

      setState(() {
        // Retire le message temporaire quelle que soit la réponse
        _messages.removeLast();
      });

      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        String reply = data['response'] ?? ''; // Réponse de l'API, vide si non disponible

        // Si la réponse est vide, afficher un message d'erreur
        if (reply.isEmpty) {
          reply = 'error_no_reply'.tr;  // Message d'erreur traduit pour réponse vide
        } else {
          // Appliquer la fonction de nettoyage à la réponse
          reply = _cleanResponse(reply);
        }

        setState(() {
          _messages.add('${'sender_speedobot'.tr} $reply');  // Ajoute la réponse nettoyée de l'API
        });
      } else {
        setState(() {
          _messages.add('${'speedobot_error_http'.tr} ${response.statusCode}');  // Erreur HTTP avec traduction
        });
      }
    } catch (e) {
      setState(() {
        _messages.removeLast(); // Retire le message temporaire en cas d'erreur
        _messages.add('${'speedobot_error_connection'.tr}: ${e.toString()}');  // Message d'erreur de connexion traduit
      });
    }
  }
}
...
Widget _buildMessageInputField() { 
  ...
  return TextField(
    ...
    decoration: InputDecoration(
      hintText: 'compose_write_essay_question'.tr,
      hintStyle: TextStyle(
