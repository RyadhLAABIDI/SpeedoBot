...
Future<void> _sendMessage() async {
  ...
  if (message.isNotEmpty) {
    ...
    await _getScienceResponse(message);
  }
}

// Fonction pour envoyer la requête à l'API et obtenir la réponse
Future<void> _getScienceResponse(String question) async {
  final url = Uri.parse('https://chat.speedobot.com/science');
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      String rawResponseMessage = data['response']?.toString() ?? '';  // Récupère la réponse ou une chaîne vide

      // Si la réponse est vide, afficher un message d'erreur professionnel
      if (rawResponseMessage.isEmpty) {
        rawResponseMessage = 'error_no_reply'.tr;  // Message d'erreur professionnel traduit
      } else {
        // Nettoyer la réponse avant de l'afficher
        rawResponseMessage = _cleanResponse(rawResponseMessage);
      }

      setState(() {
        _messages.add('speedobot : $rawResponseMessage');  // Ajoute la réponse nettoyée de l'API ou le message d'erreur
      });
    } else {
      setState(() {
...
Widget _buildMessage(String message, int index) {
  ...
  return TextField(
    ...
    decoration: InputDecoration(
      hintText: 'compose_science_question'.tr,
       hintStyle: TextStyle(
