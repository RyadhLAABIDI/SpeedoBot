...
class HumanizerScreen extends StatefulWidget {
  ...
  @override
  _HumanizerScreen createState() => _HumanizerScreen();
}

class _HumanizerScreen extends State<HumanizerScreen> {
...
Future<void> _sendMessage() async {
  ...
  if (message.isNotEmpty) {
    ...
    setState(() {
      _messages.add({
        'sender': 'Speedobot',
        'message': 'speedobot_response'.tr,  // Message de chargement
      });
    });

    final url = Uri.parse('https://chat.speedobot.com/humanizer');
    try {
      ...
      setState(() {
        _messages.removeLast();  // Retirer le message temporaire
      });

      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        String reply = data['response'] ?? '';  // Si la réponse est vide, mettre une chaîne vide
        ...
        setState(() {
          _messages.add({
            'sender': 'speedobot'.tr,
            'message': 'Erreur HTTP ${response.statusCode}',  // Message d'erreur HTTP
          });
          _isLoading = false;
        });
      }
      ...
      setState(() {
        ...
        _messages.add({
          'sender': 'speedobot'.tr,
          'message': 'Erreur de connexion: ${e.toString()}',  // Affichage de l'erreur de connexion
        });
        _isLoading = false;
      });
    }
 ...
 Widget _buildMessage(Map<String, String> messageData) {
  ...
  return TextField(
    ...
    decoration: InputDecoration(
      hintText: 'compose_humanizer_question'.tr,
      hintStyle: TextStyle(
