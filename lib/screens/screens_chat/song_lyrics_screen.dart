...
class SongLyricsScreen extends StatefulWidget {
  ...
  @override
  _SongLyricsScreen createState() => _SongLyricsScreen();
}

class _SongLyricsScreen extends State<SongLyricsScreen> {
...
Future<void> _sendMessage() async {
  ...
  if (message.isNotEmpty) {
    ...
    // API pour générer des paroles de chanson
    final url = Uri.parse('https://chat.speedobot.com/ecrire_paroles_chanson');
    try {
      ...
      if (response.statusCode == 200) {
        ...
        String reply = data['response'] ?? '';  // Clé corrigée
        ...
        setState(() {
          _messages.add('${'sender_speedobot'.tr} : Erreur HTTP ${response.statusCode}');  // Erreur HTTP
        });
      }
    } catch (e) {
      setState(() {
        _messages.removeLast(); // Retire le message temporaire en cas d'erreur
        _messages.add('${'sender_speedobot'.tr} : Erreur de connexion: ${e.toString()}');
      });
    }
  }
}
...
Widget _buildMessage(String message, int index) {
  ...
  return TextField(
    ...
    decoration: InputDecoration(
      hintText: 'enter_your_describe_song'.tr,
       hintStyle: TextStyle(
