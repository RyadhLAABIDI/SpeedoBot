...
class RecomendPlaceScreen extends StatefulWidget {
  const RecomendPlaceScreen({super.key});

  @override
  _RecomendPlaceScreen createState() => _RecomendPlaceScreen();
}

class _RecomendPlaceScreen extends State<RecomendPlaceScreen> {
...
Future<void> _sendMessage() async {
  ...
  if (message.isNotEmpty) {
    setState(() {
      _messages.add(message);  // Ajoute le message de l'utilisateur à la liste
    ...
    setState(() {
      ...
      _messages.add('speedobot_response'.tr);
    });

    // API de recommandation de lieux : recommander_lieux
    final url = Uri.parse('https://chat.speedobot.com/recommander_lieux');
    try {
      final response = await http.post(
        url,
        headers: {'Content-Type': 'application/json'},
        body: json.encode({'extra': message}),
      ).timeout(const Duration(seconds: 200)); // Timeout après 200s
      ...
      if (response.statusCode == 200) {
        ...
        String reply = data['response']?.toString() ?? ''; // Récupère la réponse ou une chaîne vide

        // Si la réponse est vide, afficher un message d'erreur professionnel
        if (reply.isEmpty) {
          reply = "error_no_reply".tr;  // Message d'erreur professionnel traduit
        } else {
          // Appliquer la fonction _cleanResponse pour nettoyer la réponse
        ...
        setState(() {
          _messages.add('Speedobot : Erreur HTTP ${response.statusCode}'.tr);  // Message d'erreur HTTP
        });
      }
    } catch (e) {
      setState(() {
        _messages.removeLast(); // Retire le message temporaire en cas d'erreur
        _messages.add('Speedobot : Erreur de connexion: ${e.toString()}'.tr); // Message d'erreur de connexion
      });
    }
  }
}
