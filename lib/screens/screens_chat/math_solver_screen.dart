...
class MathSolverScreen extends StatefulWidget {
  ...
  @override
  _MathSolverScreen createState() => _MathSolverScreen();
}

class _MathSolverScreen extends State<MathSolverScreen> {
  final TextEditingController _controller = TextEditingController();
  final ScrollController _scrollController = ScrollController();
  List<Map<String, String>> _messages = []; // Changé en List<Map>
  ...
  setState(() {
    _messages.add({'sender': 'Vous', 'message': message});  // Ajoute le message de l'utilisateur
    _isLoading = true;
  ...
  setState(() {
    _messages.add({'sender': 'speedobot'.tr, 'message': 'speedobot_response'.tr});  // Message de chargement
  });

  try {
    final response = await http.post(
      Uri.parse('https://chat.speedobot.com/mathematicien'),
      headers: {'Content-Type': 'application/json'},
      body: json.encode({'problem': message}),
    ).timeout(const Duration(seconds: 120));

    setState(() => _messages.removeLast());  // Retirer le message de chargement

    if (response.statusCode == 200) {
      final data = json.decode(response.body);
      String reply = data['response']?.toString() ?? '';  // Récupérer la réponse, ou chaîne vide si elle est vide
      ...
      setState(() {
        _messages.add({'sender': 'speedobot'.tr, 'message': reply});  // Afficher la réponse ou l'erreur
        _isLoading = false;
      });
    } else {
      setState(() {
        _messages.add({'sender': 'speedobot'.tr, 'message': 'Erreur HTTP ${response.statusCode}'});  // Message d'erreur HTTP
        _isLoading = false;
      });
    }
  } catch (e) {
    setState(() {
      ...
      _messages.add({'sender': 'speedobot'.tr, 'message': 'Erreur de connexion: ${e.toString()}'});  // Message d'erreur de connexion
      _isLoading = false;
    });
  }
}
...
String _decodeUnicode(String text) {
  ...
  Widget build(BuildContext context) {
            ...
            children: [
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) => _buildMessage(_messages[index]), // Retirer l'index
                ),
              ),
              Padding(
   ...
   // Ajout de la méthode manquante pour le fond
  ...
  // Correction des paramètres de la méthode
 Widget _buildMessage(Map<String, String> messageData) {
  ...
  return TextField(
    ...
    decoration: InputDecoration(
      hintText: 'compose_math_solver_question'.tr,
      hintStyle: TextStyle(
